...
<div x-data="budgetForm()" x-init="init()" class="max-w-4xl mx-auto">
    ...
    {{-- Estimated remaining budget card (gold border) --}}
    @if($remainingBudget > 0)
    <div class="mb-6 p-4 bg-primary-50 dark:bg-primary-900/30 rounded-xl border border-gold-200 dark:border-gold-800">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gold-200 dark:border-gold-800 overflow-hidden">
        <form method="POST" action="{{ route('budgets.store') }}" enctype="multipart/form-data" class="p-6 space-y-6" @submit="validateTotal">
            ...
            <div>
                ...
                <select x-model="copyId" @change="copyBudget" class="w-full border border-gold-300 dark:border-gold-600 dark:bg-gray-700 dark:text-white rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent">
            ...
            <div>
                ...
                <textarea name="description" x-model="form.description" rows="4" class="w-full border border-gold-300 dark:border-gold-600 dark:bg-gray-700 dark:text-white rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="date" name="start_date" x-model="form.start_date" class="w-full border border-gold-300 dark:border-gold-600 dark:bg-gray-700 dark:text-white rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">End Date</label>
                    <input type="date" name="end_date" x-model="form.end_date" class="w-full border border-gold-300 dark:border-gold-600 dark:bg-gray-700 dark:text-white rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500">
                </div>
            </div>
            ...
            <div>
                ...
                <select name="category" x-model="form.category" required class="w-full border border-gold-300 dark:border-gold-600 dark:bg-gray-700 dark:text-white rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500">
            ...
            <div>
                <div class="flex justify-between items-center mb-2">
                    ...
                    <button type="button" @click="addItem" class="text-xs bg-primary-600 hover:bg-gold-500 text-white px-3 py-1 rounded transition">+ Add Item</button>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm border border-gold-200 dark:border-gold-700">
                        ...
                        <tbody>
                            <template x-for="(item, index) in form.items" :key="index">
                                <tr>
                                    <td><input type="text" x-model="item.description" class="w-full border border-gold-300 dark:border-gold-600 dark:bg-gray-700 dark:text-white rounded px-2 py-1 text-sm focus:ring-gold-500" required></td>
                                    <td><input type="number" x-model.number="item.quantity" @input="updateTotal" class="w-24 border border-gold-300 dark:border-gold-600 dark:bg-gray-700 dark:text-white rounded px-2 py-1 text-sm focus:ring-gold-500" min="1" required></td>
                                    <td><input type="number" x-model.number="item.unit_price" @input="updateTotal" class="w-32 border border-gold-300 dark:border-gold-600 dark:bg-gray-700 dark:text-white rounded px-2 py-1 text-sm focus:ring-gold-500" min="0" step="0.01" required></td>
            ...
            <div>
                ...
                <div class="relative border-2 border-dashed border-gold-300 dark:border-gold-600 rounded-lg p-4 text-center cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition"
            ...
            <div class="flex gap-3 pt-4 border-t border-gold-200 dark:border-gold-800">
                ...
                <button type="submit" name="save_as_draft" value="1" class="flex-1 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 font-semibold py-2 px-4 rounded-lg transition">
                    Save as Draft
                </button>
                <a href="{{ route('budgets.index') }}" class="flex-1 text-center bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-lg transition">
                    Cancel
                </a>
            </div>
        </form>
