...
<div class="max-w-2xl mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
    ...
    <form method="POST" action="{{ route('financial.report.generate') }}" id="reportForm">
        ...
        <div class="flex gap-3">
            ...
            {{-- Preview is only meaningful for PDF --}}
            <button type="button"
                    id="previewBtn"
...
<script>
    // Disable the preview button when format is not PDF
    document.querySelectorAll('input[name="format"]').forEach(radio => {
        radio.addEventListener('change', () => {
            const previewBtn = document.getElementById('previewBtn');
            previewBtn.disabled = radio.value !== 'pdf';
            previewBtn.title    = radio.value !== 'pdf'
                ? 'Preview is only available for PDF'
                : '';
        });
    });
    ...
    async function previewReport(event) {
        ...
        try {
            ...
            // Re-check current format selection before re-enabling
            const currentFormat = document.querySelector('input[name="format"]:checked')?.value;
            btn.innerHTML = '👁️ Preview PDF';
            btn.disabled  = currentFormat !== 'pdf';
        }
    }
</script>
@endsection