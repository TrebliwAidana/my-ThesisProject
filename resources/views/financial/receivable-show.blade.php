...
<div class="space-y-6">
    ...
    {{-- Back Button --}}
    <div>
        <a href="{{ route('financial.receivables') }}" class="inline-flex items-center gap-2 text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200">
            ← Back to Receivables
        </a>
    </div>
    ...
    <div class="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg border border-yellow-200">
        ...
        <ul class="text-sm space-y-1 text-yellow-700 dark:text-yellow-300">
            <li>Has incomeTransaction: {{ $receivable->incomeTransaction ? 'Yes' : 'No' }}</li>
            @if($receivable->incomeTransaction)
                <li>Transaction ID: {{ $receivable->incomeTransaction->id }}</li>
                <li>Transaction status: <strong>{{ $receivable->incomeTransaction->status }}</strong></li>
                <li>Transaction is_receivable: {{ $receivable->incomeTransaction->is_receivable ? 'Yes' : 'No' }}</li>
                <li>Transaction receivable_paid: {{ $receivable->incomeTransaction->receivable_paid ? 'Yes' : 'No' }}</li>
            @endif
            <li>Receivable status: <strong>{{ $receivable->status }}</strong></li>
            <li>Receivable paid_amount: {{ $receivable->paid_amount }}</li>
            <li>Receivable total_amount: {{ $receivable->total_amount }}</li>
        </ul>
    </div>
