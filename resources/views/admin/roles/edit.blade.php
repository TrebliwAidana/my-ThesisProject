...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gold-200 dark:border-gold-800 p-6">
        ...
        @if($role->is_system)
            ...
            <div class="flex justify-end mt-6">
                <a href="{{ route('admin.roles.index') }}" class="px-5 py-2.5 text-sm font-medium rounded-lg bg-gray-500 hover:bg-gray-600 text-white transition-all duration-200 shadow-sm">Back to Roles</a>
            </div>
        ...
        @else
            ...
            <form method="POST" action="{{ route('admin.roles.update', $role->id) }}">
                ...
                {{-- Abbreviation --}}
                <div class="mb-5">
                    <label for="abbreviation" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">
                ...
                {{-- Level (direct selection 2-7) --}}
                <div class="mb-5">
                    <label for="level" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">
                    ...
                    <select id="level" name="level" required
                        ...
                        @for($i = 2; $i <= 7; $i++)
                            <option value="{{ $i }}" {{ old('level', $role->level) == $i ? 'selected' : '' }}>
                                Level {{ $i }}
                                @if($i == 2) — Supreme Admin @endif
                                @if($i == 3) — Supreme Officer @endif
                                @if($i == 4) — Club Adviser @endif
                                @if($i == 5) — Org Admin @endif
                                @if($i == 6) — Org Officer @endif
                                @if($i == 7) — Org Member @endif
                            </option>
                        @endfor
                    </select>
    ...
    {{-- Danger zone (only for non-system roles) --}}
    @if(!$role->is_system)
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-red-200 dark:border-red-900/50 p-6">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
                Deleting this role is permanent. All users with this role will need to be reassigned.
            </p>
            <button type="button"
                    onclick="confirmDelete({{ $role->id }}, '{{ addslashes($role->name) }}')"
                    class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-red-600 dark:text-red-400
