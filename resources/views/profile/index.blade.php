...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    @if(session('success'))
        <div class="p-4 bg-green-100 dark:bg-green-900/30 border border-gold-300 dark:border-gold-800 text-green-800 dark:text-green-300 rounded-xl text-sm flex items-center gap-2">
            ✅ {{ session('success') }}
        </div>
    @endif
    ...
    {{-- Profile Information Card (Emerald Header) --}}
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gold-200 dark:border-gold-800 overflow-hidden">
        <div class="bg-gradient-to-r from-emerald-600 to-emerald-700 dark:from-emerald-800 dark:to-emerald-900 px-6 py-4">
            ...
            <p class="text-sm text-emerald-100">Update your personal details</p>
        </div>
            ...
            <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <div class="flex items-center gap-4">
                        @if(auth()->user()->avatar)
                            <img src="{{ url('/secure-avatar/' . basename($user->avatar)) }}" alt="Avatar" class="w-16 h-16 rounded-full object-cover border-2 border-gold-300 dark:border-gold-600">
                        @else
                            <div class="w-16 h-16 rounded-full bg-gradient-to-br from-emerald-500 to-emerald-600 flex items-center justify-center text-white text-xl font-bold shadow-md">
                                {{ strtoupper(substr(auth()->user()->full_name, 0, 2)) }}
                            </div>
                        @endif
                        <div class="flex-1">
                            <input type="file" name="avatar" accept="image/jpeg,image/png,image/gif,image/webp"
                                   class="text-sm text-gray-600 dark:text-gray-300 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-emerald-50 file:text-emerald-700 hover:file:bg-gold-500 hover:file:text-white dark:file:bg-emerald-900/30 dark:file:text-emerald-300 dark:hover:file:bg-gold-600 transition">
    ...
    {{-- Change Password Card (Emerald Header) --}}
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gold-200 dark:border-gold-800 overflow-hidden">
        <div class="bg-gradient-to-r from-emerald-600 to-emerald-700 dark:from-emerald-800 dark:to-emerald-900 px-6 py-4">
            <h2 class="text-lg font-semibold text-white">Change Password</h2>
            <p class="text-sm text-emerald-100">Keep your account secure</p>
        </div>
                ...
                <div class="mb-4">
                    ...
                    <div class="relative">
                        <input :type="showCurrent ? 'text' : 'password'" name="current_password" required
                               class="w-full border dark:bg-gray-700 dark:text-white rounded-lg px-4 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent {{ $errors->has('current_password') ? 'border-red-500' : 'border-gold-300 dark:border-gold-600' }}">
                        <button type="button" @click="showCurrent = !showCurrent" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-500 hover:text-gold-600 dark:hover:text-gold-400 transition">
                ...
                <div class="mb-4">
                    ...
                    <div class="relative">
                        <input :type="showNew ? 'text' : 'password'" name="new_password" required
                               class="w-full border dark:bg-gray-700 dark:text-white rounded-lg px-4 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent {{ $errors->has('new_password') ? 'border-red-500' : 'border-gold-300 dark:border-gold-600' }}">
                        <button type="button" @click="showNew = !showNew" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-500 hover:text-gold-600 dark:hover:text-gold-400 transition">
                ...
                <div class="mb-6">
                    ...
                    <div class="relative">
                        <input :type="showConfirm ? 'text' : 'password'" name="new_password_confirmation" required
                               class="w-full border dark:bg-gray-700 dark:text-white rounded-lg px-4 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent {{ $errors->has('new_password_confirmation') ? 'border-red-500' : 'border-gold-300 dark:border-gold-600' }}">
                        <button type="button" @click="showConfirm = !showConfirm" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-500 hover:text-gold-600 dark:hover:text-gold-400 transition">
    ...
    {{-- Stats Card (Emerald Header) --}}
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gold-200 dark:border-gold-800 overflow-hidden">
        <div class="bg-gradient-to-r from-emerald-600 to-emerald-700 dark:from-emerald-800 dark:to-emerald-900 px-6 py-4">
            <h2 class="text-lg font-semibold text-white">Activity Summary</h2>
        </div>
        <div class="p-6 grid grid-cols-2 gap-6">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $transactionsCount ?? 0 }}</p>
            </div>
        </div>
    </div>
</div>
